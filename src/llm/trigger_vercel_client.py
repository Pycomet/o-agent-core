    ...
    def __init__(
        ...
        Args:
            ...
            api_key: Trigger.dev API key (defaults to TRIGGER_API_KEY env var)
                    Use tr_dev_* for dev, tr_prod_* for production
    ...
    async def _trigger_job(
        ...
        try:
            ...
            trigger_response = await self.client.post(
                f"/api/v1/tasks/{task_id}/trigger", json={"payload": payload}
            )
            ...
            if not run_id:
                raise ValueError(
                    f"No run ID returned from Trigger.dev. Response: {run_data}"
                )
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    status_response = await self.client.get(f"/api/v3/runs/{run_id}")
